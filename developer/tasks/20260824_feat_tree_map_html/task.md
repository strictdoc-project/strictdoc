...
StrictDoc shall provide the `tree_map` feature. The feature shall be activated
through the project configuration and generate `tree_map.html`.
...
The screen shall show one tree map canvas at a time. A view-type control in the
page header shall switch between the available maps without reloading the
...
The JSON contract shall keep a node's label and count as separate values. The
...
anchor for an SDoc node and has no anchor for a document tile. Every Document
view action shall open a new browser tab.
...
Implement the feature in `strictdoc/features/tree_map`.
...
The screen shall pass all map definitions to one renderer instance. The
renderer shall identify each map by a stable ID and keep mutable navigation and
display state separately for each ID. Changing the selected map shall restore
that map's state before rendering it.
