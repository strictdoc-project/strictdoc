...
- Each document is rendered as a rectangle containing (at minimum) the
  document title.
- An arrow is drawn between two documents whenever a node (requirement,
  section-level node, etc.) in one document has a relation (of any relation
  type/role: Parent/Child, Refine, Verify, Trace, or a custom role) to a node
  in a different document. Relations between nodes within the same document
  are not shown.
...
- Multiple relations between the same pair of documents are drawn as a
  ...
  not node-level).
- Arrow style/color varies by relation type, so Parent/Child links are
  visually distinguishable from Refine/Verify/Trace/custom-role links.
...
- Document-level cycles are not a case this feature needs to handle:
  StrictDoc's existing `TreeCycleDetector` already rejects cyclic
  Parent/Child relations at the node level during project loading, so a
  cyclic document graph cannot occur through Parent/Child relations. (Other
  relation roles — Refine/Verify/Trace/custom — are not currently
  cycle-checked by StrictDoc; this feature does not add cycle handling for
  those roles in V1. If a non-Parent/Child cycle is found in practice, it is
...
- Cycle handling for non-Parent/Child relation roles.
...
1. Walks the already-built `TraceabilityIndex` / graph database to collect,
   for every pair of documents, whether any node-to-node relation (of any
   role) crosses from one document to the other, and in which direction
   (child → parent).
...
3. Renders the result as inline SVG (rectangles for documents, arrows for
   cross-document relations, distinct styling for off-highway "skip"
   arrows and for different relation types) via a Python-side generator —
   no client-side JS graph-layout library.
...
- Cross-document relations are derived from the existing graph database
  ...
  Relation *role* (Parent/Child, Refine, Verify, Trace, custom) is read off
  each `SDocNode`'s relations the same way existing traceability screens do.
...
- Exact SVG geometry (box sizing/wrapping for long document titles, spacing
  ...
  implementation, informed by testing against `tests/integration` /
  `tests/end2end` sample projects with real multi-document relation graphs.
- Exact visual encoding for relation-type styling and for "skip" arrows
  (color vs. dash pattern vs. both) is not decided yet.
- Whether relation *role names* (not just direction) should be labeled on
  arrows in the diagram is not decided yet.
