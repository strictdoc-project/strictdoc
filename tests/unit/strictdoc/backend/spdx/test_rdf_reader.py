...
"""
    graph = Graph()
    graph.parse(data=json, format="json-ld")

    # print(graph.serialize(format="turtle"))

    pkg = URIRef(
        "https://swinslow.net/spdx-examples/example1/hello-v3-specv3/SPDXRef-gnrtd4"
    )
    for p, o in graph.predicate_objects(subject=pkg):
        print(p, o)

    SPDX = Namespace("http://spdx.org/rdf/terms#")  # example

    for verification in graph.objects(pkg, SPDX.verifiedUsing):
        print("Verification node:", verification)