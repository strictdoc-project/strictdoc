...
class Test_TC003_UI_viewtype_selector(BaseCase):
    def test_01(self):
        ...
        with SDocTestServer(
            ...
            screen_deep_traceability.assert_on_screen()

            # go to traceability
            screen_traceability = (
                viewtype_selector.do_go_to_traceability()
            )
            screen_traceability.assert_on_screen()

            # go to table
            screen_table = (
                viewtype_selector.do_go_to_table()
            )
            screen_table.assert_on_screen()

            # go back to document
            screen_document_2step = (
                viewtype_selector.do_go_to_document()
            )
            screen_document_2step.assert_on_screen()
