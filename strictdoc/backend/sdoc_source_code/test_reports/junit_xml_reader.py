...
class JUnitXMLReader:
    ...
    def read_from_string(
        ...
        xml_testsuites: Optional[
            Union[
                bs4.element.PageElement,
                bs4.element.Tag,
                bs4.element.NavigableString,
            ]
        ] = soup.find("testsuites", recursive=False)
        ...
        for xml_testsuite_ in xml_testsuite_list:
            ...
            total_tests = xml_testsuite_["tests"]
            total_failures = xml_testsuite_["failures"]
...
"""
            ...
            for xml_testcase_ in xml_testcase_list:
                ...
                xml_testcase_name: str
                xml_testcase_classname: str
                if xml_format == JUnitXMLFormat.LLVM_LIT:
                    ...
                    xml_testcase_name = assert_cast(xml_testcase_["name"], str)
                    xml_testcase_classname = assert_cast(
                        xml_testcase_["classname"], str
                    )
                    xml_testcase_time = xml_testcase_["time"]
                    ...
                    xml_error_or_none: Optional[Any] = xml_testcase_.find(
                        "error"
                    )
                    xml_failure_or_none: Optional[Any] = xml_testcase_.find(
                        "failure"
                    )
                    xml_skipped_or_none: Optional[Any] = xml_testcase_.find(
                        "skipped"
                    )

                    if xml_error_or_none is not None:
                        raise RuntimeError(
                            f"JUnit XML contains a test that failed due to an error: {xml_testcase_name}"
                        )
