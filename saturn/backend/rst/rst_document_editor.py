...
from saturn.backend.rst.rst_document_validator import RSTDocumentValidator
from saturn.backend.rst.rst_node_finder import RSTNodeFinder, LEVEL_MAX
from saturn.backend.rst.rst_parser import RSTParser
from saturn.backend.rst.rst_reader import RSTReadVisitor
...
class RSTDocumentEditor:
    ...
    def _replace_section_with_section(old_section_title, new_section):
        ...
        current_level = old_section[SATURN_ATTR_LEVEL]
        new_level = new_section[SATURN_ATTR_LEVEL]
        ...
        if new_level > current_level:
            ...
            for index, child in enumerate(old_section):
                if isinstance(child, docutils.nodes.section):
                    if child[SATURN_ATTR_LEVEL] <= new_level:
            ...
            old_section[SATURN_ATTR_LEVEL] = new_level
            ...
            while (cursor != old_section.document and
                   cursor[SATURN_ATTR_LEVEL] > new_level):
    ...
    def _replace_paragraph_with_section(old_paragraph, new_section):
        ...
        old_level = old_parent[SATURN_ATTR_LEVEL]
        new_level = new_section[SATURN_ATTR_LEVEL]
        ...
        for old_parent_child in old_parent[split_index + 1:]:
            if (isinstance(old_parent_child, docutils.nodes.section) and
                    old_parent_child[SATURN_ATTR_LEVEL] <= new_level):
                break
