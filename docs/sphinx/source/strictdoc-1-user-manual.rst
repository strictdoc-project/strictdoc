...
StrictDoc has an support for Excel XLS on input and Excel XLSX on output.

On input, the headers of sheet1 are used to put together a custom grammar and the requirements are imported one row
per requirement. A best effort is made by the importer to recognized names of headers and map these to strictdoc
requirement fields.
...
.. code-block:: text
    ...
    strictdoc import excel sdoc input.xls output.sdoc
