...
- `[x]` A node type with a required SingleChoice field is disabled.
- `[x]` Created REQUIREMENT nodes receive a generated UID.
- `[x]` The client receives the generated MID and locates the created row.
- `[ ]` Required String fields are populated with `TBD`.
- `[ ]` Create more than one supported grammar element type.
- `[ ]` A type that remains completely empty after generated/default values is
  disabled.
...
- `[x]` `add_table_node_disables_required_single_choice_type`
