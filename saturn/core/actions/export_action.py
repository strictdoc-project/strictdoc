...
from saturn.core.document_finder import DocumentFinder
from saturn.core.document_meta import DocumentMeta
from saturn.core.traceability_index import TraceabilityIndex
from saturn.export.html.export import SingleDocumentTableHTMLExport, SingleDocumentTraceabilityHTMLExport
from saturn.export.html.generators.document import DocumentHTMLGenerator
from saturn.export.html.generators.document_tree import DocumentTreeHTMLGenerator
from saturn.export.html.renderer import SingleDocumentFragmentRenderer
from saturn.export.rst.export import SingleDocumentRSTExport
from saturn.helpers.file_modification_time import get_file_modification_time
from saturn.helpers.file_system import sync_dir
from saturn.helpers.timing import timing_decorator, measure_performance
...
class ExportAction:
    def __init__(self, root_path):
        ...
        strict_own_files = glob.iglob('{}/saturn/**/*'.format(self.root_path), recursive=True)
        strict_own_files = [f for f in strict_own_files if f.endswith('.html') or f.endswith('.py')]
        latest_saturn_own_file = max(strict_own_files, key=os.path.getctime)
        self.saturn_last_update = get_file_modification_time(latest_saturn_own_file)
    ...
    def export(self, path_to_single_file_or_doc_root):
        ...
        static_files_src = os.path.join(self.root_path, 'saturn/export/html/static')
    ...
    def _export_with_performance(self, document, document_tree, traceability_index):
        ...
        if os.path.isfile(full_output_path):
            ...
            if (sdoc_mtime < output_file_mtime and
                self.saturn_last_update < output_file_mtime):
