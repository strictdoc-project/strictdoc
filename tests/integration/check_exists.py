...
main_parser.add_argument('--invert',
                         action='store_true',
                         default=False,
                         help='Enforce checking that input_path is a file')
...
if not invert:
    ...
    if os.path.exists(args.input_path):
        print('error: expected path to not exist, but it does: {}'.format(args.input_path),
              file=sys.stderr)
        exit(1)
    if args.file and os.path.isfile(args.input_path):
        print('error: expected path to not exist, but is a file: {}'.format(args.input_path),
              file=sys.stderr)
        exit(1)
    if args.dir and os.path.isdir(args.input_path):
        print('error: expected path to not exist, but is a directory: {}'.format(args.input_path),
              file=sys.stderr)
        exit(1)
exit(0)
