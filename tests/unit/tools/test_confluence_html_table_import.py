...
def test_01():
    html_file = os.path.join(FIXTURES_PATH, '001-confluence-html-file-two-sections/001-confluence-html-table-two-sections.html')
...
def test_02():
    html_file = os.path.join(FIXTURES_PATH, '002-confluence-html-file-parsing-paragraphs/002-confluence-html-table-parsing-paragraphs.html')
    sdoc = ConfluenceHTMLTableImport.import_from_file(html_file)
    ...
    assert section1_reqs[0].comments[0].comment_multiline == 'REQ-1-1 comment wrapped in p'
    assert section1_reqs[1].comments[0].comment_multiline == '- Item 1\n- Item 2'
...
def test_fragments_03():
    html_content = "<html><td><ul><li>Item 1</li><li>Item 2</li></ul></td></html>"
    soup = BeautifulSoup(html_content, "xml")
    print(soup)
    td = soup.find("td")
    assert td
